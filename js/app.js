  ...
  try{
    ...
    if(!teachers.length){
      ...
      teachersBody.innerHTML = teachers.map(t=>{
        const args = [t.key, t.uid||'', t.email||'', t.name||''].map(v=>JSON.stringify(v)).join(',');
        return `<tr>
          ...
          <td><button type="button" class="action-btn danger" style="font-size:12px;padding:4px 10px"
            onclick="adminDeleteTeacherAccount(${args})">🗑️ ${isEn?'Delete':'حذف'}</button></td>
        </tr>`;
      }).join('');
    }
...
async function adminDeleteTeacherAccount(key, uid, email, name){
  const isEn = currentLang==='en';
  const label = name || email || key;
  if(!key){
    showToast(isEn ? '❌ Invalid teacher' : '❌ معلم غير صالح');
    return;
  }
  if(!confirm(isEn
    ? `Delete teacher "${label}" permanently?\nThis removes their account, email login, and all app data.`
    : `حذف المعلم "${label}" نهائياً؟\nسيُحذف حسابه وبريده وكلمة مروره وجميع بياناته.`)) return;

  try{
    if(typeof firebase !== 'undefined' && firebase.functions){
      const fn = firebase.app().functions('us-central1').httpsCallable('adminDeleteTeacher');
      await fn({ key, uid: uid || null });
      showToast(isEn ? '✅ Teacher deleted completely' : '✅ تم حذف المعلم بالكامل');
      adminLoadMonitoring();
      return;
    }
  }catch(e){
    console.warn('adminDeleteTeacher cloud function:', e);
  }
  ...
  try{
    const updates = {};
    updates['teachers/'+key] = null;
    updates['teacherData/'+key] = null;
    updates['publicTeachers/'+key] = null;
    if(uid) updates['teacherLookup/'+uid] = null;
    await db.ref().update(updates);
    showToast(isEn
      ? '✅ Teacher data deleted. Deploy Cloud Functions for full Auth account removal.'
      : '✅ تم حذف بيانات المعلم. لحذف البريد نهائياً: npm run delete:teacher');
    adminLoadMonitoring();
  }catch(err){
    console.error(err);
