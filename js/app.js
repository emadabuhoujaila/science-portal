...
function renderOverview(){
  ...
  document.getElementById('overview-tbody').innerHTML=list.map((s,i)=>{
    ...
    const diag=(rec.diagnostic||s.diagnostic||0);
    const t1=(s.t1||0),t2=(s.t2||0),hw=(s.hw||0),portal=(s.portal||0),
          activity=(s.activity||0),total=(s.total||0),lab=(s.lab||0),finalG=(rec.final||s.final||0);
...
function renderGradesTab(){
  ...
  const rows = students.map((s,i)=>{
    ...
    return `<tr>
      ...
      <td>${(rec.diagnostic||0) ? (rec.diagnostic||0).toFixed(1) : '—'}</td>
      <td>${(s.t1||0).toFixed(1)}</td><td>${(s.t2||0).toFixed(1)}</td>
      ${weekCells(rec.hwWeeks)}
      ${weekCells(rec.portalWeeks)}
      ${weekCells(rec.actWeeks)}
      <td>${s.lab ?? '—'}</td>
      <td><strong style="color:${c}">${(s.total||0).toFixed(1)}</strong></td>
      <td>${(rec.final||0) ? (rec.final||0).toFixed(1) : '—'}</td>
  ...
  if(students.length){
    ...
    const rows2 = rows + `<tr class="avg-row">
