...
function syncParentTeacherMessages(tc, cls, sName, snap){
  ...
  const mine = vals.filter(m => m && m.cls === cls && (m.name || '').trim() === sName);
  APP.messages = (APP.messages || []).filter(m => m._src !== tag);
  mine.forEach(m => APP.messages.push({ ...m, _src: tag }));
  saveState();
}
...
function syncParentTeacherBehavior(tc, cls, sName, snap){
  const tag = tc.key + '|bv';
  const vals = snap.exists() ? Object.values(snap.val() || {}) : [];
  const mine = vals.filter(e => e && e.cls === cls && (e.name || '').trim() === sName);
  APP.behaviorLog = (APP.behaviorLog || []).filter(e => e._src !== tag);
  mine.forEach(e => APP.behaviorLog.push({ ...e, _src: tag }));
  saveState();
}
...
function clearParentMessages(){
  ...
  window.fbClearParentMessages(key)
    .then(()=>{
      try{ localStorage.removeItem('del_pm_'+key); }catch(e){}
      APP.parentMessages = [];
      saveState();
      renderParentInbox();
      updateInboxBadge();
