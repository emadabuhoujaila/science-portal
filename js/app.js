...
async function renderParentAcademic(cls, studentName, mid, teachersList){
  ...
  for(const tc of teachersList){
    ...
    try{
      ...
      const s = arr.find(x=>String(x.mid)===String(mid)||x.name===studentName);
...
async function loadSubjectTabContent(idx, cls, studentName, mid, teachersList){
  ...
  if(typeof db!=='undefined'){
    try{
      ...
      if(gSnap.exists()){
        ...
        gradeData = arr.find(s=>String(s.mid)===String(mid)||s.name===sName);
        if(!gradeData&&mid){
  ...
  const gradeHtml = gradeData ? `
    ...
    </div>` : `<div class="empty-state" style="padding:20px;margin-bottom:14px">
      <div class="ico">📊</div>
      <p style="font-size:13px">${isEn?'Grades not entered yet for this subject':'لم تُدخل درجات هذه المادة بعد'}</p>
    </div>`;
    ...
    try{
      ...
      const findStudentByName = (name)=>{
        for(const g of ['5','6','7','8']){
          for(const s of getGradeStudents(g)){
            if(s.name === name) return { grade:g, student:s };
          }
        }
        return null;
      };

      const saveGradeRow = (name, row)=>{
        const hit = findStudentByName(name);
        if(!hit) return false;
        const existing = hit.student;
        const grade = hit.grade;
        const sec   = (existing.section || '').toUpperCase();
        ...
        if(typeof db !== 'undefined' && tKey){
          db.ref(`teacherData/${tKey}/grades/${bucket}/${mId}`).set(gradeObj).catch(err=>{
            console.warn('grade save', err);
          });
        }
        return true;
      };

      // Legacy sheets (7-1, 7-2, …) — map old class keys then resolve by name across grades
      const sheetMap = {'7-1':'1','7-2':'2','7-4':'4','7-5':'5'};
      wb.SheetNames.forEach(shName=>{
        if(!sheetMap[shName]) return;
        const ws = wb.Sheets[shName];
        const rows = XLSX.utils.sheet_to_json(ws, {header:1, defval:''});
        let dataStart = -1;
        for(let i=0;i<rows.length;i++){
          const cell = String(rows[i][2]||'').trim();
          if(/[\u0600-\u06FF]/.test(cell) && cell.length > 5 && findStudentByName(cell)){
            dataStart = i; break;
          }
        }
        ...
        for(let i=dataStart; i<rows.length; i++){
          ...
          if(saveGradeRow(name, rows[i])) imported++;
        }
      });

      // First sheet fallback (any layout with names in col C)
      if(!imported && wb.SheetNames.length){
        const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], {header:1, defval:''});
        for(const row of rows){
          const name = String(row[2]||'').trim();
          if(!name || !/[\u0600-\u06FF]/.test(name)) continue;
          if(saveGradeRow(name, row)) imported++;
        }
      }

      if(imported>0){
        showToast(isEn?`✅ Updated ${imported} students`:`✅ تم تحديث ${imported} طالب من الملف`);
        renderAllTabs();
      } else {
        showToast(isEn?'⚠️ Could not match students — check names in Excel':'⚠️ لم يتم التعرف على الطلاب — تأكد من الأسماء في Excel');
      }
    } catch(err){
      console.error(err);
      showToast('❌ '+(isEn?'File error: ':'خطأ في قراءة الملف: ')+err.message);
    }
    input.value='';
  };
  reader.readAsArrayBuffer(file);
