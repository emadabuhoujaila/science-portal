...
// ══════════════════════════════════════════════════
//  GRADE EXCEL IMPORT — official template
// ══════════════════════════════════════════════════
...
function findStudentByMid(mid, gradeHint, sectionHint){
  ...
  for(const g of grades){
    if(!g) continue;
    const trySections = sectionHint
      ? [normalizeSectionCell(sectionHint)]
      : [null];
    for(const sec of trySections){
      const list = sec ? getGradeStudents(g, sec) : getGradeStudents(g);
...
function findGradeDataStartRow(rows, map){
  for(let i=3; i<rows.length; i++){
    const mid = String(rows[i]?.[map.mid] ?? '').trim();
...
function parseGradeRow(row, map){
  const mid = String(row[map.mid] ?? '').trim();
  const gradeHint = normalizeGradeCell(row[map.grade]);
  const sectionHint = normalizeSectionCell(row[map.section]);
  const nameAr = String(row[map.nameAr] ?? '').trim();
  if(!mid) return { error:'empty_mid', mid, name:nameAr };

  const hit = findStudentByMid(mid, gradeHint, sectionHint);
  if(!hit) return { error:'no_match', mid, name:nameAr };
  ...
  reader.onload = async function(e){
    try{
      ...
      for(let i=start; i<rows.length; i++){
        const row = rows[i];
        const midRaw = String(row[map.mid] ?? '').trim();
        if(!midRaw) continue;
        if(!/^\d+$/.test(midRaw.replace(/\s/g,''))) continue;

        const parsed = parseGradeRow(row, map);
        if(parsed.error === 'no_match'){
          unmatched.push({ mid: midRaw, name: parsed.name || '—' });
          continue;
        }
        ...
        const g = hit.grade;
        const sec = (hit.student.section || '').toUpperCase();

        if(scope){
          if(scope.grades?.length && !scope.grades.includes(g)) continue;
          if(scope.gradeMap?.[g] && sec && !scope.gradeMap[g].includes(sec)) continue;
        }
      ...
      if(imported > 0){
        showToast(isEn ? `✅ Updated ${imported} student(s)` : `✅ تم تحديث ${imported} طالب`);
        renderAllTabs();
      } else {
        showToast(isEn ? '⚠️ No grades imported' : '⚠️ لم يتم استيراد أي درجات');
      }

      if(unmatched.length){
        const lines = unmatched.slice(0, 15).map(u =>
          (isEn ? `ID ${u.mid}` : `رقم ${u.mid}`) + (u.name && u.name !== '—' ? ` — ${u.name}` : '')
        ).join('\n');
        const more = unmatched.length > 15 ? `\n... +${unmatched.length - 15}` : '';
        alert(
          (isEn
            ? `⚠️ ${unmatched.length} student ID(s) not found in admin list (ministerial numbers):\n\n`
            : `⚠️ ${unmatched.length} رقم طالب غير موجود في قائمة المسؤول (الرقم الوزاري):\n\n`)
          + lines + more
        );
      }
    } catch(err){
