...
function syncPublicTeacher(key, teacher){
  ...
  return db.ref('publicTeachers/'+key).set(buildPublicTeacherProfile(teacher));
}
  ...
  if(typeof auth !== 'undefined' && auth){
    auth.createUserWithEmailAndPassword(email, pw)
      .then(cred=>{
        ...
        updates['publicTeachers/' + key] = buildPublicTeacherProfile(teacherData);
...
function applyTeacherProfile(){
  ...
  if(te) te.textContent = CURRENT_TEACHER.name + ' | ' + subjLabel;
  const ts = document.getElementById('teacher-school');
  if(ts){
    const scope = getTeacherScope();
    const gradesStr = scope ? scope.grades.map(g=>(isEn?'G':'ص')+g).join(' ') : (isEn?'All':'الكل');
    const secsStr   = scope ? scope.sections.join(' ') : (isEn?'All':'الكل');
    ts.textContent = isEn
      ? 'Grade School · '+subjLabel+' | Grades: '+gradesStr+' | Sec: '+secsStr
      : 'المدرسة · '+subjLabel+' | الصفوف: '+gradesStr+' | الشعب: '+secsStr;
  }
  // Refresh grade dropdowns for this teacher's scope
...
function checkPin(){
  ...
  if(entered === correct){
    // حفظ جلسة ولي الأمر للدخول التلقائي لاحقاً
    APP.savedParent = { cls: pendingLogin.cls, name: pendingLogin.name, section: pendingLogin.section||'', mid: pendingLogin.mid||'' };
    saveState();
    showScreen('parent');
    loadParentSubjectTabs(pendingLogin.cls, pendingLogin.name, entered);
...
function renderOverview(){
  ...
  const list=allStudents(cls,sec);
  ...
  document.getElementById('overview-tbody').innerHTML=list.map((s,i)=>{
    const tot=(s.total||0); const c=tot>=80?'#1a9a9a':tot>=70?'#c8961e':'#e53935';
    const t1=(s.t1||0),t2=(s.t2||0),hw=(s.hw||0),portal=(s.portal||0),
          activity=(s.activity||0),total=(s.total||0);
    const sec=s.section?` <span style="font-size:11px;color:var(--grey-3)">${s.section}</span>`:'';
    return `<tr>
      <td>${i+1}</td>
      <td><span class="badge badge-teal">${s.cls}</span>${sec}</td>
...
function renderGradesTab(){
  ...
  document.getElementById('grades-title').textContent = isEn
    ? `📝 Grades - Class ${cls}`
    : `📝 درجات الشعبة ${cls}`;
  let students = allStudents(cls, sec);
  const rows = students.map((s,i)=>{
    ...
    return `<tr>
      ...
      <td>${(s.hw||0).toFixed(1)}%</td><td>${(s.portal||0).toFixed(1)}%</td>
      <td>${s.activity}%</td><td>${s.lab}</td>
  ...
  // Average row
  if(students.length){
    const avg = f=>( students.reduce((a,s)=>a+s[f],0)/students.length ).toFixed(1);
    ...
    rows2 = rows + `<tr style="background:var(--teal-pale);font-weight:700">
      <td>—</td><td style="text-align:${isEn?'left':'right'}">${avgLabel}</td>
      <td>${avg('t1')}</td><td>${avg('t2')}</td>
      <td>${avg('hw')}%</td><td>${avg('portal')}%</td>
      <td>${avg('activity')}%</td><td>—</td>
      <td style="color:${avgColor}">${avg('total')}</td>
      <td>—</td>
    </tr>`;
    document.getElementById('grades-tbody').innerHTML = rows2;
  } else {
    document.getElementById('grades-tbody').innerHTML = rows;
  }
}
...
function setTpl(k){ document.getElementById('msg-body').value=TPLS[k]; updatePreview(); }
function updatePreview(){
  const name=document.getElementById('msg-student').value||'الطالب';
  const body=document.getElementById('msg-body').value;
  const type=document.getElementById('msg-type').value;
  const icons={praise:'🌟',warning:'⚠️',info:'📘'};
  document.getElementById('msg-preview').textContent=
    `${icons[type]} رسالة من معلم العلوم\n\nولي أمر الطالب: ${name}\n\n${body}\n\n— معلم العلوم · الصف السابع`;
}
...
const TRANSLATIONS = {
  ar: {
    // Login screen
    appTitle: 'بوابة العلوم',
    schoolName: 'مدرسة محمد بن حمد الشرقي حلقة ثانية - بنين',
    grade: 'الصف السابع · مادة العلوم',
    teacher: 'معلم المادة: عماد ممدوح أبو حجيلة',
    tabTeacher: '👨‍🏫 المعلم',
    tabParent: '👨‍👦 ولي الأمر',
    ...
    teacherSchool: 'مدرسة محمد بن حمد الشرقي · علوم الصف السابع',
    teacherName: 'أ. عماد ممدوح أبو حجيلة',
  ...
  en: {
    appTitle: 'Science Portal',
    schoolName: 'Mohamed Bin Hamad Al Sharqi School - Boys',
    grade: 'Grade 7 · Science',
    teacher: 'Teacher: Emad Mahmoud Abu Hujaila',
    tabTeacher: '👨‍🏫 Teacher',
    tabParent: '👨‍👦 Parent',
    ...
    teacherSchool: 'Mohamed Bin Hamad Al Sharqi School · Grade 7 Science',
    teacherName: 'Mr. Emad Mahmoud Abu Hujaila',
...
function applyGlobalLang(){
  ...
  setText('login-grade',        'grade');
  setText('parent-teacher',     'teacher');
  ...
  setText('parent-grade',    'grade');
  setText('parent-teacher',  'teacher');
  const ti=document.getElementById('tab-btn-inbox');
  if(ti) ti.textContent=t('inbox');
  const tb=document.getElementById('tab-btn-bvlog');
  if(tb) tb.textContent=t('bvlog');
  const to=document.getElementById('tab-btn-outbox');
  if(to) to.textContent=t('outbox');
...
function applyGradesLang(){
  ...
  // Table headers
  const grHeaders = {
    'gr-th-name':  isAr?'الاسم':'Name',
    'gr-th-t1':    isAr?'ت1':'Form.1',
    'gr-th-t2':    isAr?'ت2':'Form.2',
    'gr-th-hw':    isAr?'واجبات%':'HW%',
    'gr-th-portal':isAr?'بوابة%':'Portal%',
    'gr-th-act':   isAr?'نشاط%':'Activity%',
    'gr-th-lab':   isAr?'مختبر':'Lab',
    'gr-th-total': isAr?'المحصلة':'Total',
    'gr-th-grade': isAr?'التقدير':'Grade',
  };
  Object.entries(grHeaders).forEach(([id,txt])=>{ const el=document.getElementById(id); if(el) el.textContent=txt; });

  // Re-render table (flips names + title + average row)
}
...
function applyMessagesLang(){
  ...
  // Preview
  setText('msg-preview-lbl', isAr?'معاينة':'Preview');
  const prevPH = document.getElementById('msg-preview-ph');
  if(prevPH) prevPH.textContent = isAr?'اختر طالباً وأكتب رسالة...':'Select student and write message...';
...
function toggleLang(){
  try {
    ...
    if(screenId === 'screen-teacher'){
      ...
      try{ renderParentView(window._currentParent.cls, window._currentParent.name); }catch(e){}
    }
...
function _startParentListeners(cls, studentName, mid, teachersList){
  window._parentListeners.forEach(ref=>ref.off());
  window._parentListeners = [];
  ...
  teachersList.forEach((tc,i)=>{
    ...
    msgRef.on('value', snap=>{
      ...
      vals.filter(m=>m&&m.cls===cls&&(m.name||'').trim()===sName).forEach(m=>{
        if(!APP.messages.find(x=>x.ts===m.ts&&x.body===m.body)) APP.messages.push(m);
      });
      const myMsgs = vals.filter(m=>m&&m.cls===cls&&(m.name||'').trim()===sName);
    ...
    bvRef.on('value', snap=>{
      if(!snap.exists()) return;
      const vals = Object.values(snap.val()||{});
      vals.filter(e=>e&&e.cls===cls&&(e.name||'').trim()===sName).forEach(e=>{
        if(!APP.behaviorLog.find(x=>x.ts===e.ts)) APP.behaviorLog.push(e);
      });
      const myLogs = vals.filter(e=>e&&e.cls===cls&&(e.name||'').trim()===sName);
    ...
    pmRef.on('value', snap=>{
      ...
      Object.values(snap.val()||{}).forEach(m=>{
        if(m&&m.cls===cls&&(m.name||'').trim()===sName)
          if(!APP.parentMessages.find(x=>x.ts===m.ts)) APP.parentMessages.push(m);
      });
    });
...
function renderParentSubjectTabs(cls, studentName, mid, teachersList, section){
  ...
  // Start listeners for badges
  setTimeout(()=>_startParentListeners(cls, studentName, mid, teachersList), 400);
}
...
async function renderParentAcademic(cls, studentName, mid, teachersList){
  ...
  // Collect grades from each teacher
  let allGrades = []; // [{subject, subjLabel, teacher, t1, t2, hw, portal, activity, lab, total}]

  for(const tc of teachersList){
    if(typeof db === 'undefined') break;
    try{
      const section = window._currentParent?.section||'';
      const snap = await db.ref(`teacherData/${tc.key}/grades/${cls}${section}`).once('value');
      if(!snap.exists()) continue;
      const gd = snap.val();
      const arr = Array.isArray(gd)?gd:Object.values(gd);
      const s = arr.find(x=>x.mid===mid||x.name===studentName);
      if(s) allGrades.push({...s, subject:tc.subject, subjLabel:tc.subjLabel, teacherName:tc.name});
    }catch(e){}
  }

  const hasData = allGrades.some(g=>g.total);
  const isEnL   = currentLang==='en';

  // Build summary table
  const tableRows = teachersList.map(tc=>{
    const g = allGrades.find(x=>x.subject===tc.subject);
    const total  = g?(g.total||0):null;
    const color  = total===null?'var(--grey-3)':total>=80?'var(--green-soft)':total>=70?'var(--gold)':'var(--red-soft)';
    const badge  = total===null?`<span style="color:var(--grey-3);font-size:12px">${isEnL?'Not entered':'لم يُدخل'}</span>`
  ...
  // Overall average
  const enteredGrades = allGrades.filter(g=>g.total);
  ...
  div.innerHTML = `
    ...
    <div class="card-header" style="margin-bottom:8px">
      <h4>${isEnL?'📊 Academic Report':'📊 التقرير الأكاديمي'}</h4>
    </div>
    <div class="table-wrap">
      <table>
        <thead><tr>
          ...
          <th>${isEnL?'Test 1':'ت1'}</th>
          <th>${isEnL?'Test 2':'ت2'}</th>
          <th>${isEnL?'HW':'الواجبات'}</th>
          <th>${isEnL?'Activity':'النشاط'}</th>
          <th>${isEnL?'Total':'المجموع'}</th>
        </tr></thead>
        <tbody>${tableRows||`<tr><td colspan="7" style="text-align:center;color:var(--grey-3);padding:20px">
          ${isEnL?'No grades entered yet':'لم تُدخَل الدرجات بعد'}</td></tr>`}
        </tbody>
      </table>
    </div>`;
}
  ...
  div.innerHTML = msgs.length
    ? msgs.map(m=>`
        <div class="msg-card" style="margin-bottom:10px">
          <div class="msg-header">
            <span class="msg-type ${m.type||'info'}">${mi[m.type]||'📘'} ${t((m.type||'info')+'Type')||''}</span>
            <span class="msg-date">${m.date||''}</span>
          </div>
          <p style="margin:4px 0 0;font-size:13px">${m.body||''}</p>
        </div>`).join('')
      ...
      .then(snap=>{
        ...
        Object.values(snap.val()||{}).forEach(m=>{
          ...
          if((m.name||'').trim()===sName)
            allSent.push({...m,_teacher:tc.name,_subj:tc.subjLabel||''});
        });
      }).catch(()=>{})
  );
        ...
        <div style="background:var(--white);border:1px solid var(--grey-5);
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:6px">
            <span style="font-size:11px;font-weight:700;color:var(--teal-mid)">
              📤 ${isEn?'To':'إلى'}: ${m._teacher||'المعلم'}${m._subj?' · '+m._subj:''}
            </span>
            <span style="font-size:11px;color:var(--grey-3)">${m.date||''}</span>
          </div>
          <p style="margin:0;font-size:13px;color:var(--grey-2);white-space:pre-line">${m.body}</p>
        </div>`).join('')
...
async function loadSubjectTabContent(idx, cls, studentName, mid, teachersList){
  window._parentSubjectContext = {cls,name:studentName,mid,teachers:teachersList};
  ...
  if(typeof db!=='undefined'){
    try{
      const [gSnap, mSnap, bSnap] = await Promise.all([
        db.ref('teacherData/'+tc.key+'/grades/'+cls+section).once('value'),
        db.ref('teacherData/'+tc.key+'/messages').once('value'),
        db.ref('teacherData/'+tc.key+'/behaviorLog').once('value'),
      ]);
      if(gSnap.exists()){
        const arr = Object.values(gSnap.val()||{});
        gradeData = arr.find(s=>s.mid===mid||s.name===sName);
        if(!gradeData&&mid){
          const ms = await db.ref('teacherData/'+tc.key+'/grades/'+cls+section+'/'+mid).once('value');
          if(ms.exists()) gradeData=ms.val();
        }
      }
      if(mSnap.exists()){
        messages = Object.values(mSnap.val()||{})
  ...
  // Merge into APP for behavior/messages tabs
  messages.forEach(m=>{
    if(!APP.messages.find(x=>x.ts===m.ts&&x.body===m.body)) APP.messages.push(m);
  });
  behaviorLog.forEach(e=>{
    if(!APP.behaviorLog.find(x=>x.ts===e.ts)) APP.behaviorLog.push(e);
  });
  ...
  const gradeHtml = `
    <div class="breakdown-grid" style="margin-bottom:14px">
      <div class="breakdown-item"><span class="ico">📝</span><div class="info"><div class="name">${t('t1')}</div><div class="score">${(gd.t1||0).toFixed(1)}%</div></div></div>
      <div class="breakdown-item"><span class="ico">📝</span><div class="info"><div class="name">${t('t2')}</div><div class="score">${(gd.t2||0).toFixed(1)}%</div></div></div>
      <div class="breakdown-item"><span class="ico">📚</span><div class="info"><div class="name">${t('hw')}</div><div class="score">${(gd.hw||0).toFixed(1)}%</div></div></div>
      <div class="breakdown-item"><span class="ico">🖥️</span><div class="info"><div class="name">${t('portal')}</div><div class="score">${(gd.portal||0).toFixed(1)}%</div></div></div>
      <div class="breakdown-item"><span class="ico">🏫</span><div class="info"><div class="name">${t('activity')}</div><div class="score">${(gd.activity||0)}%</div></div></div>
      <div class="breakdown-item"><span class="ico">🔬</span><div class="info"><div class="name">${t('lab')}</div><div class="score">${(gd.lab||0)}%</div></div></div>
    </div>
    ${gradeData?`<div style="text-align:center;background:var(--teal-pale);border-radius:8px;padding:10px;margin-bottom:14px">
      ...
      +[...messages].reverse().map(m=>`
        <div class="msg-card" style="margin-bottom:8px">
          <div class="msg-header">
            ...
            <span class="msg-date">${m.date||''}</span>
          </div><p style="margin:4px 0 0">${m.body}</p>
        </div>`).join('')
  ...
  container.innerHTML = gradeHtml + bvHtml + msgsHtml + `
    <div class="parent-msg-box" style="margin-top:14px">
      ...
      <button class="parent-msg-send"
        onclick="sendParentMsgToTeacher('${idx}','${tc.key}','${cls}','${studentName}')">
...
async function sendParentMsgToTeacher(idx, teacherKey, cls, studentName){
  ...
  if(!body){
    ...
    if(ta) ta.style.borderColor='var(--red-soft)';
    return;
  }
  const isEn = currentLang==='en';
  const d = new Date();
  const msg = {
    cls, name:studentName, body,
    date: d.toLocaleDateString('ar-AE')+' '+d.toLocaleTimeString('ar-AE',{hour:'2-digit',minute:'2-digit'}),
    ts: d.toISOString()
  };

  if(typeof db!=='undefined' && teacherKey){
    await window.fbPushParentMsg(teacherKey, msg).catch(()=>{});
  }
  APP.parentMessages = APP.parentMessages||[];
  APP.parentMessages.push(msg);
  saveState();
...
function renderSubjectSentLog(idx, teacherKey, cls, studentName){
  ...
  const isEn = currentLang==='en';
  const sName= studentName.trim();
  // Read from APP.parentMessages
  const myMsgs = (APP.parentMessages||[]).filter(m=>{
    if(!m||m.cls!==cls) return false;
    return (m.name||'').trim()===sName;
  }).slice().reverse();
...
function refreshParentInbox(cls, name){
  ...
  if(!div) return;
  const myMsgs = APP.messages.filter(m => m.name===name && m.cls===cls);
  const icons  = {praise:'🌟', warning:'⚠️', info:'📘'};
...
function deleteSingleBvEntry(id, cls, name){
  ...
  if(id){ APP.behaviorLog = APP.behaviorLog.filter(e=>e.id!==id); }
  else   { const i=APP.behaviorLog.findIndex(e=>e.cls===cls&&e.name===name); if(i>=0) APP.behaviorLog.splice(i,1); }
  saveState(); renderBvLog();
  if(id && window.fbDeleteSingleBvEntry){
    window.fbDeleteSingleBvEntry(id).then(()=>showToast('✅ تم حذف السجل'));
  } else { showToast('✅ تم حذف السجل'); }
}
...
function refreshParentPage(){
  // Restore _currentParent from savedParent if needed
  ...
  // Reload subject tabs (async - no .then needed)
...
// ══════════════════════════════════════════════════
// Rich message templates for parent messages
const PARENT_MSG_TEMPLATES = {
  ar: {
    q1: (name) => `السلام عليكم ورحمة الله وبركاته
أستفسر عن درجات ابني ${name} في مادة العلوم.
...
وجزاكم الله خيراً على حرصكم وتفانيكم.`,
    q4: (name) => `السلام عليكم ورحمة الله وبركاته
ابني ${name} يواجه بعض الصعوبات في مادة العلوم وأرى أنه يحتاج إلى دعم إضافي.
...
جزاكم الله خيراً وبارك في مسيرتكم التعليمية النبيلة.`
  ...
  en: {
    q1: (name) => `Dear Teacher,
I would like to inquire about ${name}'s grades in Science class.
...
Thank you for your care and commitment to our children.`,
    q4: (name) => `Dear Teacher,
${name} is facing some difficulties in Science and may need extra support.
...
function selectQuick(btn, key){
  ...
  const tplFn = PARENT_MSG_TEMPLATES[lang]?.[key];
  const text = tplFn ? tplFn(displayName) : t(key);
  document.getElementById('parent-msg-input').value = text;
}
...
function renderParentSentLog(cls, name){
  ...
  if(!div) return;
  const isEn = currentLang === 'en';
  // Filter parentMessages for this student
  const myMsgs = (APP.parentMessages||[])
    .filter(m => m.cls===cls && m.name===name)
  ...
  html += myMsgs.map((m, i) => {
    ...
    return '<div style="background:var(--white);border:1px solid var(--grey-5);border-right:3px solid var(--teal-soft);border-radius:10px;padding:12px 14px;margin-bottom:8px">'
      ...
      + '</div>'
      + '<p style="font-size:13px;color:var(--grey-2);line-height:1.6;margin:0;white-space:pre-line">'+body+'</p>'
      + '</div>';
  }).join('');
...
function sendParentMessage(cls, name){
  const body = document.getElementById('parent-msg-input').value.trim();
  if(!body){
    document.getElementById('parent-msg-input').style.borderColor='var(--red-soft)';
    setTimeout(()=>document.getElementById('parent-msg-input').style.borderColor='',1500);
    return;
  }
  const d = new Date();
  const msg = {
    cls, name, body, read: false,
    date: d.toLocaleDateString('ar-AE')+' '+d.toLocaleTimeString('ar-AE',{hour:'2-digit',minute:'2-digit'})
  };
  if(window.fbPushParentMsg){
    window.fbPushParentMsg(msg).then(ok => {
      if(!ok){ APP.parentMessages.push(msg); saveState(); }
    });
  } else {
    APP.parentMessages.push(msg); saveState();
  }
  ...
  // Update sent log
  setTimeout(()=>{
    if(window._currentParent) renderParentSentLog(cls, name);
  }, 300);
}
...
function deleteSingleParentMsg(id){
  ...
  // Step 3: Mark remaining as seen (no new badge after delete)
  markAllParentMsgsSeen();
  renderParentInbox();
  updateInboxBadge();
...
function renderParentInbox(){
  if(typeof markAllParentMsgsSeen === "function") markAllParentMsgsSeen();
  ...
  wrap.innerHTML = msgs.map(m=>`
    <div class="inbox-card ${m.read?'':'unread'}">
      <div class="inbox-header">
        <div style="cursor:pointer;flex:1" onclick="markRead('${m.id||''}')">
          <div class="inbox-name">${(currentLang==="en" && (getGradeStudents(m.cls)||[]).find(s=>s.name===m.name)?.nameEn) || m.name} — ${currentLang==="en"?"Class":"الشعبة"} ${m.cls}</div>
          <div class="inbox-date">${m.date||m.ts||''}</div>
        </div>
        <div style="display:flex;align-items:center;gap:6px">
          ${m.read?'':'<div class="unread-dot" title="${currentLang===\"en\"?\"Unread\":\"غير مقروءة\"}"></div>'}
          ${m.id?`<button class="action-btn danger" style="padding:2px 8px;font-size:11px" onclick="deleteSingleParentMsg('${m.id}')">🗑️</button>`:''}
        </div>
      </div>
...
function markRead(id){
  // id is the Firebase key (e.g. -NxYZ...) or array index
  const msg = typeof id === 'string'
    ? APP.parentMessages.find(m=>m.id===id)
    : APP.parentMessages[id];
  if(!msg) return;
  if(window.fbMarkParentRead && msg.id){
    window.fbMarkParentRead(msg.id);
  } else {
    msg.read = true; saveState(); renderParentInbox();
  }
}

function clearParentMessages(){
  if(!APP.parentMessages.length){ showToast('لا توجد رسائل'); return; }
  if(!confirm('هل تريد حذف جميع رسائل أولياء الأمور؟\nسيُحذف من كل الأجهزة.')) return;
  APP.parentMessages = []; saveState(); renderParentInbox();
  if(window.fbClearParentMessages) window.fbClearParentMessages().then(()=>showToast('✅ تم مسح رسائل أولياء الأمور'));
  else showToast('✅ تم المسح');
}

function clearTeacherMessages(){
  if(!APP.messages.length){ showToast('لا توجد رسائل'); return; }
  if(!confirm('هل تريد حذف جميع رسائل المعلم المحفوظة؟\nسيُحذف من كل الأجهزة.')) return;
  APP.messages = []; saveState(); renderSavedMessages();
  if(window.fbClearTeacherMessages) window.fbClearTeacherMessages().then(()=>showToast('✅ تم مسح رسائل المعلم'));
  else showToast('✅ تم المسح');
}

function clearAllBehavior(){
  if(!APP.behaviorLog.length){ showToast('لا توجد سجلات'); return; }
  if(!confirm('هل تريد مسح كامل سجل السلوك لجميع الطلاب؟\nسيُحذف من كل الأجهزة ومن لوحات أولياء الأمور.')) return;
  APP.behaviorLog = []; APP.behavior = {}; saveState(); renderBvLog();
  if(window.fbClearAllBehavior) window.fbClearAllBehavior().then(()=>showToast('✅ تم مسح كامل سجل السلوك'));
  else showToast('✅ تم المسح');
}
...
function markAllParentMsgsSeen(){
  const key = getTeacherKey();
  if(!key) return;
  try{ localStorage.setItem('seen_pm_'+key, new Date().toISOString()); }catch(e){}
  updateInboxBadge();
}
...
function updateInboxBadge(){
  const key = getTeacherKey();
  let seenTs = '';
  try{ seenTs = key ? (localStorage.getItem('seen_pm_'+key)||'') : ''; }catch(e){}
  const msgs = APP.parentMessages||[];
  // Only count messages newer than last seen time
  const newCnt = seenTs
    ? msgs.filter(m=>(m.ts||'')>seenTs).length
...
function importExcel(input){
  ...
  reader.onload = function(e){
    try{
      ...
      let imported = 0;
      // Map sheet names to class keys
      const sheetMap = {'7-1':'1','7-2':'2','7-4':'4','7-5':'5'};

      wb.SheetNames.forEach(shName => {
        const cls = sheetMap[shName];
        if(!cls) return;
        const ws   = wb.Sheets[shName];
        const rows = XLSX.utils.sheet_to_json(ws, {header:1, defval:''});

        // Find header row: look for a row where col[2] looks like a name (Arabic text)
        let dataStart = -1;
        for(let i=0;i<rows.length;i++){
          const cell = String(rows[i][2]||'');
          // Arabic name rows: contains Arabic chars and is in STUDENTS list
          if(/[\u0600-\u06FF]/.test(cell) && cell.length > 5){
            const match = getGradeStudents(cls).find(s=>s.name===cell.trim());
        ...
        for(let i=dataStart; i<rows.length; i++){
          ...
          const roster = getGradeStudents(cls);
          const idx  = roster.findIndex(s=>s.name===name);
          if(idx < 0) continue;
          const existing = roster[idx];

          const t1    = parseFloat(row[3])||0;
          const t2    = parseFloat(row[4])||0;

          const hw       = parseFloat(row[15])*100||parseFloat(row[14])||0;
          const portal   = parseFloat(row[24])*100||parseFloat(row[23])||0;
          const activity = parseFloat(row[35])*100||parseFloat(row[34])||0;
          const lab      = parseFloat(row[36])||100;
          const total    = parseFloat(row[37])||parseFloat(row[38])||0;
          ...
          const gradeObj = {
            ...
            t1:   +t1.toFixed(1),
            t2:   +t2.toFixed(1),
            hw:   +(hw>1?hw:hw*100).toFixed(1),
            portal:+(portal>1?portal:portal*100).toFixed(1),
            activity:+(activity>1?activity:activity*100).toFixed(0),
            lab:  +lab.toFixed(0),
            total:+(total>1?total:total*100).toFixed(1)
          };
          const sec  = existing.section || '';
          const mId  = existing.mid || name.replace(/ /g,'_');
          if(!window.TEACHER_GRADES[cls + sec]) window.TEACHER_GRADES[cls + sec] = {};
          window.TEACHER_GRADES[cls + sec][mId] = gradeObj;
          if(typeof db !== 'undefined' && CURRENT_TEACHER && CURRENT_TEACHER.email){
            const tKey = emailKey(CURRENT_TEACHER.email);
            db.ref(`teacherData/${tKey}/grades/${cls}${sec}/${mId}`).set(gradeObj).catch(()=>{});
          }
          updated++;
        }
        imported += updated;
      ...
      if(imported>0){
        showToast(`✅ تم تحديث ${imported} طالب من الملف`);
        renderAllTabs();
      } else {
        showToast('⚠️ لم يتم التعرف على البيانات — تأكد من هيكل الملف');
      }
    } catch(err){
      console.error(err);
...
function tryAutoLogin(){
  ...
  const sp = APP.savedParent;
  showScreen('parent');
  renderParentView(sp.cls, sp.name);
}
